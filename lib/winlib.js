...
class WinCont {
    ...
    modal_open(p) {
        ...
        // Execute any <script> tags included in the inserted message HTML.
        // Browsers do not run scripts inserted via innerHTML, so clone and append them
        // to the document to ensure event handlers inside modal message run.
        try {
            const msgContainer = document.getElementById(`${MW}_message`);
            if (msgContainer) {
                const scripts = Array.from(msgContainer.querySelectorAll('script'));
                console.log('modal_open: detected script tags:', scripts.length);
                scripts.forEach((oldScript, idx) => {
                    try {
                        const preview = oldScript.src ? oldScript.src : (oldScript.textContent || oldScript.innerText || '').slice(0, 120).replace(/\n/g, ' ');
                        console.log(`modal_open: script[${idx}] preview:`, preview);
                    } catch (e) { console.warn('modal_open: script preview error', e); }
                    ...
                    if (oldScript.src) {
                        ...
                        // append to head so external script is fetched and executed
                        document.head.appendChild(newScript);
                        // remove it after load to keep DOM clean
                        newScript.addEventListener('load', () => { try { newScript.remove(); } catch (e) {} });
                    } else {
            ...
            console.warn('modal_open: script execution failed', err);
        }
    }
