...
class Maplibre {
    ...
    init(Conf) {
        return new Promise((resolve, reject) => {
            ...
            // オフライン時は PMTiles スタイルを優先、無い場合は極小スタイル
            const offlineBlank = {
                version: 8,
                name: 'offline-blank',
                sources: {},
            ...
            let styleToUse = this.styles[this.selectStyle];
            if (!navigator.onLine) {
                const offlineName = (Conf.map && Conf.map.offlineTileName) ? Conf.map.offlineTileName : undefined;
                if (offlineName && this.styles[offlineName]) {
                    console.log(`Maplibre: offline, using style "${offlineName}"`);
                    styleToUse = this.styles[offlineName];
                } else {
                    console.log('Maplibre: offline, using blank fallback style');
                    styleToUse = offlineBlank;
                }
            }
            this.map = new maplibregl.Map({
                container: 'mapid', style: styleToUse, "maxZoom": Conf.map.maxZoom, "zoom": Conf.map.initZoom,
